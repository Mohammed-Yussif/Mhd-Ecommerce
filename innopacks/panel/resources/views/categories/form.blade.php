...
<form class="needs-validation" novalidate id="app-form"
  ...
  <div class="row">
            ...
            <div class="accordion-item">
              ...
              <div id="data-locale-{{ $localeCode }}"
                ...
                <div class="accordion-body">
                  ...
                  <x-common-form-input title="title" name="translations[{{$localeCode}}][name]"
                    value="{{ old('translations.' . $localeCode . '.name', $category->translate($localeCode, 'name')) }}"
                    required placeholder="title" />

                  <x-common-form-textarea title="content" name="translations[{{$localeCode}}][content]"
    ...
    <div class="col-12 col-md-3 ps-md-0">
      <div class="card">
        <div class="card-body">
          <x-common-form-switch-radio title="state" name="active" :value="old('active', $category->active ?? true)"
            placeholder="state" />
          <x-common-form-input title="{{ __('panel::common.slug') }}" name="slug" :value="old('slug', $category->slug ?? '')"
            placeholder="{{ __('panel::common.slug') }}" />
          <x-common-form-select title="Parent Category" name="parent_id" :value="old('parent_id', $catalog->parent_id ?? 0)"
            :options="$categories" key="id" label="name" />
          <x-common-form-input title="Sorting" name="position" :value="old('position', $category->position ?? 0)"
            placeholder="Sorting" />
